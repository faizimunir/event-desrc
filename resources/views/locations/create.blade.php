<x-layouts::app :title="__('Add Location')" :unified-header="true">
    <div class="flex h-full w-full flex-1 flex-col">
        <x-admin-detail-hero
            :heading="__('Add Location')"
            :subheading="__('Locations')"
            :back-href="route('locations.index')"
        />

        <div class="users-hero-content flex flex-1 flex-col gap-4 pt-4 pb-6">
            <form method="post" action="{{ route('locations.store') }}" class="max-w-lg space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <flux:textarea name="google_map" rows="4" placeholder="https://www.google.com/maps/... or embed iframe src">{{ old('google_map') }}</flux:textarea>
                    @error('google_map')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400" role="alert">{{ $message }}</p>
                    @enderror
                </div>
