<x-layouts::app :title="__('Edit Track')" :unified-header="true">
    <div class="flex h-full w-full flex-1 flex-col">
        ...
        <div class="users-hero-content flex flex-1 flex-col gap-4 pt-4 pb-6">
            ...
            @canAs('track.delete')
                @can('delete', $track)
                    <form id="delete-track-form-{{ $track->id }}" method="post" action="{{ route('events.tracks.destroy', [$event, $track]) }}" class="mt-2">
                        @csrf
                        @method('DELETE')
                        <flux:button
                            type="button"
