<x-layouts::app :title="__('Edit Package')" :unified-header="true">
    <div class="flex h-full w-full flex-1 flex-col">
            ...
            <div class="relative space-y-3 px-4 pb-3 pt-[max(0.5rem,env(safe-area-inset-top))] sm:px-5 sm:pb-4 lg:space-y-3.5 lg:pt-4">
                ...
                <div class="hidden items-center justify-between gap-3 lg:flex">
                    ...
                    <flux:button
                        variant="ghost"
                        size="sm"
                        :href="route('events.show', [$event, 'tab' => 'packages'])"
                        wire:navigate
                        icon="arrow-left"
        ...
        <div class="users-hero-content flex flex-1 flex-col gap-4 pt-4 pb-6">
            ...
            @canAs('package.delete')
                @can('delete', $package)
                    <form id="delete-package-form-{{ $package->id }}" method="post" action="{{ route('events.packages.destroy', [$event, $package]) }}" class="mt-2">
                        @csrf
                        @method('DELETE')
                        <flux:button
                            type="button"
