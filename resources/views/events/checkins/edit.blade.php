...
@endphp
<x-layouts::app :title="__('Edit check-in')" :unified-header="true">
    <div class="flex h-full w-full flex-1 flex-col">
        ...
        <div class="users-hero-content flex flex-1 flex-col gap-4 pt-4 pb-6">
            ...
            @canAs('checkin.delete')
                @can('delete', $checkin)
                    <form id="delete-checkin-form-{{ $checkin->id }}" method="POST" action="{{ route('events.checkins.destroy', [$event, $checkin]) }}" class="mt-2">
                        @csrf
                        @method('DELETE')
                        <flux:button
                            type="button"
