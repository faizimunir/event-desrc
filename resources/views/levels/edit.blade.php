<x-layouts::app :title="__('Edit Level')" :unified-header="true">
    <div class="flex h-full w-full flex-1 flex-col">
        <x-admin-detail-hero
            :heading="__('Edit Level')"
        ...
        <div class="users-hero-content flex flex-1 flex-col gap-4 pt-4 pb-6">
            ...
            @canAs('level.delete')
                @can('delete', $level)
                    <form id="delete-level-form-{{ $level->id }}" method="post" action="{{ route('levels.destroy', $level) }}" class="mt-2">
                        @csrf
                        @method('DELETE')
                        <flux:button
                            type="button"
