<x-layouts::app :title="__('Edit Reward')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <flux:heading>{{ __('Edit Reward') }}</flux:heading>
        ...
        @canAs('reward.delete')
            @can('delete', $reward)
                <form id="delete-reward-form-{{ $reward->id }}" method="post" action="{{ route('rewards.destroy', $reward) }}" class="mt-6">
                    @csrf
                    @method('DELETE')
                    <flux:button
                        type="button"
