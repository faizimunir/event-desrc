<x-layouts::app :title="__('Edit Team')" :unified-header="true">
    <div class="flex h-full w-full flex-1 flex-col">
        <x-admin-detail-hero
            :heading="__('Edit Team')"
            :subheading="__('Teams')"
        ...
        <div class="users-hero-content flex flex-1 flex-col gap-4 pt-4 pb-6">
            ...
            @canAs('team.delete')
                @can('delete', $team)
                    <form id="delete-team-form-{{ $team->id }}" method="post" action="{{ route('teams.destroy', $team) }}" class="mt-2">
                        @csrf
                        @method('DELETE')
                        <flux:button
                            type="button"
